...
var myWork = {
  ...
  if((typeof instance=== 'undefined' || typeof encoded_auth=== 'undefined') || (instance.length === 0 || encoded_auth.length === 0)){
    ...
    if(debug){
      console.log('DEBUG url: ' + encodeURI('https://' + instance + '.service-now.com/api/now/table/task?sysparm_query=assigned_to.user_name=' + user_name + '^active=true'));
  ...
  ajax(
    ...
    var content = 'Created By: ' + data.result[e.itemIndex].sys_created_by + '\n' +
      ...
      //Insert something here to for example add a menu option to remove yourself as the assignee
